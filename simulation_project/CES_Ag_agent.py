...
class Agent:
    ...
    def learn(self, experiences, gamma):
        ...
        Q_targets = reward + discount_gamma * critic_target(next_state, actor_target(next_state))
        recall:
            actor_target = action value function, state -> value, (distribution over actions)
            critic_target = state value function, (state, action) -> new_state -> value
    ...
    def update_memory(self, end_reward):
        """At end of episode, update pre_memory tuples with end reward then add tuples to memory."""
        for memory in self.pre_memory:
            state, action, old_reward, next_state, done = memory
            self.memory.add(state, action, end_reward, next_state, done)
